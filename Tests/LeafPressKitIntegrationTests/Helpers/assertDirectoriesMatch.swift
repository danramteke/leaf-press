    ...
    do {
      ...
      if let actualContentsString = String(data: actualContents, encoding: .utf8)?
         ...
         let expectedContentsString = String(data: expectedContents, encoding: .utf8)?
          .trimmingCharacters(in: .whitespacesAndNewlines) {
        XCTAssertEqual(actualContentsString, expectedContentsString, "Actual contents do not match: `diff \(actualPath.string) \(expectedPath.string)`", file: file, line: line)
      } else {
